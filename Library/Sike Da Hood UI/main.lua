...
--// REFERENCES
local Main, TabScroll, Content, ControlFrame, SideLayout
local Tabs = {}
local CurrentTab
...
Main.InputBegan:Connect(function(i)
    if Library.State.canDrag and (i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch) then
        dragging = true
        startPos = i.Position
...
UIS.InputChanged:Connect(function(i)
    if dragging and Library.State.canDrag and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
...
function Library:Init()
    ...
    SidebarLogo.TextColor3 = self.Theme.Accent
    ...
    TabScroll.Size = UDim2.new(1,0,1,-140)
    TabScroll.Position = UDim2.new(0,0,0,50)
    TabScroll.BackgroundTransparency = 1
    TabScroll.BorderSizePixel = 0
    TabScroll.ScrollBarThickness = 4
    TabScroll.ScrollBarImageColor3 = self.Theme.Sub
    ...
    ControlFrame.BackgroundColor3 = self.Theme.Panel
    ...
    CreateLightBtn(self.Theme.Red,spacing,"red")
    CreateLightBtn(self.Theme.Green,spacing*2+btnSize+8,"green")
    ...
    branding.TextColor3 = self.Theme.Accent
    branding.Parent = bottomFrame
end

function Library:Toggle()
    if self.State.mode == "red" then return end
    self.State.Open = not self.State.Open
    Main.Visible = self.State.Open
    
    TweenService:Create(Blur,TweenInfo.new(0.3,Enum.EasingStyle.Quad),{
        Size = self.State.Open and 18 or 0
    }):Play()
end
...
function Library:NewTab(name)
    ...
    return {
        Name = name,
        Container = frame,
        AddLabel = function(txt) self:AddLabel(frame, txt) end,
        AddToggle = function(txt, callback) self:AddToggle(frame, txt, callback) end,
        AddSlider = function(txt, min, max, def, callback) self:AddSlider(frame, txt, min, max, def, callback) end,
        AddDropdown = function(txt, opts, callback) self:AddDropdown(frame, txt, opts, callback) end
    }
end
...
--// ELEMENTS
function Library:AddLabel(parent, txt)
    ...
    lbl.TextSize = 14
    lbl.TextColor3 = self.Theme.Sub
    lbl.TextXAlignment = Enum.TextXAlignment.Left
    lbl.Parent = parent
end

function Library:AddToggle(parent, txt, callback)
    local holder = Instance.new("Frame")
    holder.Size = UDim2.new(1,0,0,32)
    holder.BackgroundTransparency = 1
    holder.Parent = parent

    self:AddLabel(holder, txt)
    ...
    switch.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            TweenService:Create(knob,TweenInfo.new(0.25,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
                ...
                BackgroundColor3 = state and Color3.fromRGB(0,0,0) or self.Theme.Text
            }):Play()
...
function Library:AddSlider(parent, txt, min, max, default, callback)
    self:AddLabel(parent, txt)
    ...
    valueText.TextColor3 = self.Theme.Text
    ...
    track.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            sliderDragging = true
            self.State.canDrag = false -- Stop window drag
        end
    end)
    ...
    UIS.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            self.State.canDrag = true -- Resume window drag
        end
    end)
...
function Library:AddDropdown(parent, txt, options, callback)
    local isOpen = false
    self:AddLabel(parent, txt)
